...
def validate():
    ...
    # 0. Provider mode for Lab 24 local fallback
    print("Model providers:")
    llm_provider = os.getenv("LLM_PROVIDER", "auto")
    embedding_provider = os.getenv("EMBEDDING_PROVIDER", "auto")
    lmstudio_base_url = os.getenv("LMSTUDIO_BASE_URL", "http://localhost:1234/v1")
    has_openai = bool(os.getenv("OPENAI_API_KEY") or os.getenv("OPEN_API_KEY"))
    print(f"  LLM_PROVIDER={llm_provider}")
    print(f"  EMBEDDING_PROVIDER={embedding_provider}")
    print(f"  LMSTUDIO_BASE_URL={lmstudio_base_url}")
    if not has_openai and llm_provider in {"auto", "lmstudio"}:
        print("  INFO: OPENAI_API_KEY not set; chat calls will use LM Studio local fallback.")
    if not has_openai and embedding_provider == "auto":
        print("  INFO: Embeddings will try LM Studio, then sentence-transformers fallback.")
