...
def _get_openai_client():
    ...
    try:
        ...
        warnings.warn("[pipeline] openai package not installed — falling back to context[0].")
        return None
...
def run_query(query: str, search: HybridSearch, reranker: CrossEncoderReranker) -> tuple[str, list[str]]:
    ...
    if contexts:
        try:
                ...
                [
                    {
                        "role": "system",
                        "content": (
                            "Bạn là trợ lý trả lời câu hỏi CHỈ dựa trên context được cung cấp. "
                            "Trả lời ngắn gọn, chính xác bằng tiếng Việt. "
                            "Nếu context không chứa thông tin để trả lời → nói 'Không tìm thấy thông tin.'"
                        ),
                    },
            ...
            warnings.warn(f"[pipeline] LLM generation failed: {e} — falling back to context[0].")
